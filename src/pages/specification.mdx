...
Clients start each turn by sending a JSON object with `type: "response.create"`. The remaining fields follow the standard response creation request body, except transport-specific HTTP fields such as `stream`, `stream_options`, and `background` MUST NOT be sent on WebSocket requests. Servers MUST treat a WebSocket `response.create` message as a request to create a response on the active connection.
...
A single WebSocket connection MUST process at most one in-flight response at a time. Servers MAY accept multiple `response.create` messages over one connection, but they MUST process those messages sequentially unless they document a provider-specific multiplexing extension.
...
Servers MAY keep connection-local response state to make continuation lower latency. If the referenced response is not available from connection-local or persisted state, the server MUST fail the turn with an error whose code is `previous_response_not_found`.
